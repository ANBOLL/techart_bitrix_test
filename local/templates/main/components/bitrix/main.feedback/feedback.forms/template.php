...
<p>Отправьте сообщение и свои контактные данные чтобы мы могли связаться с вами!</p> -->
	...
	<form action="<?=POST_FORM_ACTION_URI?>" method="POST">
		...
		<div class="form-group">
		<label for="mainFeedback_email"><?=GetMessage("MFT_THEME")?><?
			if(empty($arParams["REQUIRED_FIELDS"]) || in_array("THEME", $arParams["REQUIRED_FIELDS"])):?><span class="mf-control-required">*</span><?endif?></label>
			<select name="theme" <?if(empty($arParams["REQUIRED_FIELDS"]) || in_array("THEME", $arParams["REQUIRED_FIELDS"])):?>required<?endif?> class="form-control">
			<option value="">(Без темы)</option>
			<?
			$arrOption = $arResult["THEME_LIST"];
